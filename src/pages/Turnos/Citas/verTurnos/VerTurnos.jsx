...
export const VerTurnos = ({ uidUsuario }) => {
    ...
    const groupByMonth = (turnos) => {
        return turnos.reduce((acc, turno) => {
            const month = new Date(turno.fecha).toLocaleString('default', { month: 'long', year: 'numeric' });
            if (!acc[month]) {
                acc[month] = [];
            }
                    ...
                    {isLoading ? (
                        ...
                        Object.keys(turnosGrouped).map((month) => (
                            <div key={month}>
                                ...
                                <div className={`contenido-mes ${openMonth === month ? 'open' : ''}`}>
                                    <ul>
                                        {turnosGrouped[month].map((turno) => (
                                            <li key={turno.id}>
                                                ...
                                                <div className="acciones-paciente">
                                                    ...
                                                    <button className='boton-borrar' onClick={() => handleDelete(turno.id)}>Eliminar</button>
                                                </div>
                                            </li>
                                        ))}
                                    </ul>
